...
function isSetComputeUnitPriceInstruction(ix) {
    return (ix.programId.equals(ComputeBudgetProgram.programId) && ix.data[0] === 3 // opcode for setComputeUnitPrice is 3
    );
}
/**
 * Check if a given transaction contains a SetComputeUnitLimit instruction
 */
export function hasSetComputeLimitInstruction(instructions) {
    return instructions.filter(isSetComputeLimitInstruction).length === 1;
}
/**
 * Check if a given transaction contains a SetComputeUnitLimit instruction
 */
export function hasSetComputeUnitPriceInstruction(instructions) {
 ...
 * ```
 */
export async function sendVersionedTransaction(connection, instructions, signers, priorityFee = 10000, lookupTables, options = {}) {
 ...
 */
export async function addComputeInstructions(connection, instructions, lookupTables, payer, priorityFee = 10000, computeUnitBuffer = {}, commitment = "confirmed") {
    ...
    if (!hasSetComputeLimitInstruction(instructions)) {
        const simulatedCompute = await getSimulationComputeUnits(connection, instructions, payer, lookupTables, commitment);
 ...
 */
export async function createLookupTable(connection, sender, additionalAddresses, priorityFee = 10000) {
